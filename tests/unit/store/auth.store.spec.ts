...
describe('auth store', () => {
  ...
  describe('actions', () => {
    ...
    it('signup registers, sets auth, and loads prefs', async () => {
      await store.actions.signup({ commit, dispatch }, {
        name: 'Test', email: 'test@test.com', password: 'pass', text: ''
      })
    ...
    it('login rethrows on failure', async () => {
      ...
      await expect(
        store.actions.login({ commit, dispatch }, { username: 'user', password: 'wrong' })
